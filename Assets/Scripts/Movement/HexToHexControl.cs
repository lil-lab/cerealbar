...
/// <summary>
/// Cached hex to hex cell movement - NO QUEUE
/// </summary>
public class HexToHexControl : ActionInformation
{
    ...
    {
      if (instructionControl.HasFocus()) { return; }
      ...
      #if UNITY_STANDALONE
      if (!timeKeeper.isPaused && !isMoving && canAct)
      #endif
        {
    ...
    private void SetMoveTo(int neighborIndex, int dir, string tag)
    {
        ...
        if (CanMove(nextLocation, dir) && turnController.CanMove(currentLocation.landType))
        {
            ...
            km.keyDownTime = timeKeeper.gameTime;
                ...
                if (turnBased) { turnController.Movement(myCharacter, currentLocation.landType); }
        }
    }
