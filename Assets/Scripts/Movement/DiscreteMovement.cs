...
/// <summary>
/// Note this is basically the queued movement script but no queue
/// Agent moves by some preset specs
/// </summary>
public class DiscreteMovement : ActionInformation {
    public bool isExecuting = false;
    private Queue<AgentAction> actionsQ = new Queue<AgentAction>();

    private List<AgentAction> actionsRecord = new List<AgentAction>();
    public delegate void OnFinishedActionHandler();
    public static event OnFinishedActionHandler OnFinishedActionEvent;

    public static void InvokeActionFinished()
    {
        OnFinishedActionEvent();
    }
    ...
    {
        if (!isExecuting ||actionsQ.Count >=1)
        {
